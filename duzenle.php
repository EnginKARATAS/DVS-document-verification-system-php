...
$sorgu = $baglanti->query("SELECT * FROM tbl_belgeler WHERE id =".(int)$_GET['id']); 
...
if ($_POST) { // Post olup olmadığını kontrol ediyoruz.
    ...
    if ($baslik<>"" && $icerik<>"") { // Veri alanlarının boş olmadığını kontrol ettiriyoruz.
        ...
        if ($baglanti->query("UPDATE tbl_belgeler SET baslik = '$baslik', icerik = '$icerik' WHERE id =".$_GET['id'])) 
        {
